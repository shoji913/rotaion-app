...
for (let s = 0; s < CONFIG.DISPLAY_SLOTS; s++) {
  ...
  const isHour = s % 2 === 0;
  line.style.cssText = `position:absolute;left:${s * cwH}px;width:${cwH}px;top:0;bottom:0;border-left:1px ${s % 2 === 0 ? 'solid var(--border2)' : 'dashed var(--border)'};box-sizing:border-box;`;
...
function buildLaneRow(lane: number, laneItems: { staffId: string; shiftIdx: number; start: number; end: number }[], dateStr: string): HTMLElement {
  ...
  for (let s = 0; s < CONFIG.DISPLAY_SLOTS; s++) {
    const line = document.createElement('div');
    line.style.cssText = `position:absolute;left:${s * cw}px;width:${cw}px;top:0;bottom:0;border-left:1px ${s % 2 === 0 ? 'solid var(--border2)' : 'dashed var(--border)'};box-sizing:border-box;`;
